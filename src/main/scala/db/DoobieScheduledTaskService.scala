...
import rozklad.api.NoScheduledTasksWereAcquired

import java.time.Instant

class DoobieScheduledTaskService[F[_]](xa: Transactor[F], observer: Observer[F])(implicit ME: MonadCancel[F, Throwable]) extends ScheduledTaskService[F] {
  ...
  }.transact(xa).flatTap(tasks => if(tasks.isEmpty) observer.occurred(NoScheduledTasksWereAcquired) else observer.occurred(ScheduledTasksAcquired(tasks)))

  override def done(id: Id[ScheduledTask], now: Instant): F[ScheduledTask] = {
    for {
      task <- ScheduledTaskRepository.done(id, now).validateAndGet(id)
      _ <- ScheduledTaskLogRepository.insert(List(task))
    } yield task
  ...
  override def logs(id: Id[ScheduledTask]): F[List[ScheduledTaskLog]] = {
    ScheduledTaskLogRepository.logs(id)
      .transact(xa)
  }
  ...
  implicit class RichListOfTasks(fa: doobie.ConnectionIO[List[ScheduledTask]]) {
    def validateAndGet(id: Id[ScheduledTask]): doobie.ConnectionIO[ScheduledTask] = {
      fa.ensure(TaskIsNotInExpectedStatusException(id))(_.size == 1)
        .map(_.head)
    }
  }
